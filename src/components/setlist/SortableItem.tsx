...
export function SortableItem({
  ...
  const CardContent = (
    <div
      className={`relative flex items-center justify-between px-3 py-2 bg-card rounded-xl shadow-sm border border-border border-l-4 transition-colors h-full
      ...
      <div className="flex items-center gap-3 overflow-hidden w-full">
        ...
        <div className="shrink-0 w-10 h-10 flex items-center justify-center">
          {isEditMode ? (
            ...
            // 通常モード :　トラック番号
            <div
              className={`w-8 h-8 rounded-full border flex items-center justify-center shadow-sm bg-muted border-border font-black text-foreground text-sm`}
            >
              {index + 1}
            </div>
          )}
      ...
      <div className="flex items-center justify-center w-16 shrink-0">
        {isEditMode ? (
          ...
          <div
            {...attributes}
            {...listeners}
            className="text-muted-foreground cursor-grab active:cursor-grabbing p-2 touch-none outline-none hover:text-foreground transition-colors"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
