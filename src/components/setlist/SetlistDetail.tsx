  ...
  return (
    <div className="pb-20">
      ...
      <div className="fixed top-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 h-16 flex items-center justify-center">
        <div className="w-full max-w-md px-4 flex items-center justify-between relative">
          ...
          <div className="flex items-center gap-1 z-10 w-24"> 
            {isEditMode ? (
              <button onClick={toggleEditMode} className="text-sm font-bold text-amber-500 px-2 py-1">完了</button>
            ) : (
              <>
                <button onClick={() => setIsAddModalOpen(true)} className="p-2 -ml-2 text-amber-500 hover:bg-amber-50 rounded-full transition">
                ...
                <button onClick={toggleEditMode} className="p-2 text-gray-400 hover:bg-gray-100 rounded-full transition">
          ...
          <div className="ml-auto z-10 w-24 flex justify-end">
             {isEditMode ? (
               <button onClick={handleBulkDelete} disabled={selectedIds.length === 0} className={`p-2 rounded-full transition ${selectedIds.length > 0 ? "text-red-500 hover:bg-red-50" : "text-gray-300"}`}>
               ...
               <button onClick={() => router.push("/setlists")} className="p-2 -mr-2 text-gray-400 hover:text-gray-600 rounded-full hover:bg-gray-100 transition block">
      ...
      <div className="pt-16">
        ...
        {items.length === 0 && (
           <div className="text-center py-10 text-gray-400 font-bold">曲がありません。<br/>左上の「＋」ボタンで追加してね！</div>
        )}
      </div>
