      ...
      <div className="space-y-2">
        ...
        {!isImpossible && !isOriginalKeyPerfect && adjDown !== null && (
          <div className="flex items-center justify-between text-xs font-bold bg-blue-50 dark:bg-blue-900/30 px-3 py-2 rounded-lg border-2 border-blue-200 dark:border-blue-800/50 text-blue-800 dark:text-blue-300 animate-in zoom-in">
            <span className="flex items-center gap-1">
              <span className="text-sm">⬇️</span> オク下で歌うなら
            </span>
            {adjDown === currentKey ? (
              <span className="bg-blue-100 dark:bg-blue-800/50 text-blue-600 dark:text-blue-200 px-3 rounded-full text-[10px] border border-blue-200 dark:border-blue-700 min-w-27.5 h-8 flex items-center justify-center">
                今のキーでOK!
              </span>
            ) : (
              <button
        ...
        {!isImpossible && !isOriginalKeyPerfect && adjUp !== null && (
          <div className="flex items-center justify-between text-xs font-bold bg-rose-50 dark:bg-rose-900/30 px-3 py-2 rounded-lg border-2 border-rose-200 dark:border-rose-800/50 text-rose-800 dark:text-rose-300 animate-in zoom-in">
            <span className="flex items-center gap-1">
              <span className="text-sm">⬆️</span> オク上で歌うなら
            </span>
            {adjUp === currentKey ? (
              <span className="bg-rose-100 dark:bg-rose-800/50 text-rose-600 dark:text-rose-200 px-3 rounded-full text-[10px] border border-rose-200 dark:border-rose-700 min-w-27.5 h-8 flex items-center justify-center">
                今のキーでOK!
              </span>
            ) : (
              <button
        ...
        {/* 3. 普通に歌う提案 */}
        {!isImpossible &&
          !isOriginalKeyPerfect &&
          adjNormal !== null &&
          adjNormal !== currentKey && (
