...
export default function SingingRecordSection({ songId, records = [] }: Props) {
  ...
  const chartData = [...records]
    ...
    .map((r) => ({
      date: new Date(r.createdAt).toLocaleDateString("ja-JP", { month: "short", day: "numeric" }),
      score: r.score,
    }));
    ...
    <div className="mt-8 border-t border-border-light pt-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-black text-foreground flex items-center gap-2">
          カラオケ日記 <span className="text-sm font-normal text-muted-foreground ml-2">({records.length}件)</span>
        </h2>
        <button
          onClick={toggleForm}
      ...
      {isOpen && (
        <form onSubmit={handleSubmit} className="bg-muted/30 border border-border-light p-5 rounded-xl mb-6 animate-in fade-in slide-in-from-top-2 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            
            <div>
              <label className="block text-xs font-bold text-muted-foreground mb-1">点数 (任意)</label>
              <div className="relative">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm font-bold pointer-events-none">点</span>
              </div>
              {isScoreOver100 && (
            ...
            <div>
              <label className="block text-xs font-bold text-muted-foreground mb-1">キー (任意)</label>
              <div className="relative">
                <select
                  value={key}
          ...
          <div>
            <label className="block text-xs font-bold text-muted-foreground mb-1">振り返りメモ (任意)</label>
            <textarea
              rows={3}
              value={memo}
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                ...
                <Tooltip 
                  ...
                  formatter={(value: number | undefined) => [`${value ?? 0} 点`, 'スコア']}
      ...
      <div className="space-y-3">
        {records.length === 0 ? (
          ...
          records.map((record) => (
            <div key={record.id} className="bg-card border border-border rounded-xl p-4 shadow-sm hover:border-primary/30 transition-colors group relative overflow-hidden">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                <span className="text-xs font-bold text-muted-foreground bg-muted px-2 py-1 rounded-md">
                  {new Date(record.createdAt).toLocaleDateString("ja-JP", { year: "numeric", month: "short", day: "numeric" })}
                </span>
              ...
              <div className="flex items-end gap-4 mb-2">
                {record.score !== null && (
                  <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-black text-foreground leading-none">{record.score}</span>
                    <span className="text-sm font-bold text-muted-foreground">点</span>
                  </div>
                )}
                {record.key !== null && (
                  <div className="text-sm font-bold text-primary bg-primary/10 px-2 py-1 rounded-md mb-0.5">
                    キー: {record.key > 0 ? `+${record.key}` : record.key === 0 ? "±0" : record.key}
                  </div>
                )}
              </div>
